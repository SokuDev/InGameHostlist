...
namespace Hostlist
{
        ...
        if (!offline) {
            if (newTime - oldTime >= delayTime) {
                try {
                    ...
                    if (e == 429) Status::Error("Rate limit reached");
                    else Status::Error("Error code: " + to_string(e));
                    delayTime = LONG_WAITTIME;
                }
	...
	void Render() {
        if (SokuAPI::GetCMenuConnect()->Choice != 6) {
            ...
            ImGui::Begin("HostList##Hosts", 0, ImGuiWindowFlags_NoMouseInputs | ImGuiWindowFlags_NoSavedSettings | ImGuiWindowFlags_NoBackground | ImGuiWindowFlags_NoTitleBar | ImGuiWindowFlags_NoMove | ImGuiWindowFlags_NoResize | ImGuiWindowFlags_NoBringToFrontOnFocus | ImGuiWindowFlags_NoNavInputs | ImGuiWindowFlags_NoNav | ImGuiWindowFlags_NoNavFocus);
            ...
            // Pretty much a duplicate, I should find a way to streamline it 
                ...
                ImGui::SetColumnWidth(2, 32);
                ImGui::Separator();
                ImGui::Text("Player 1"); ImGui::NextColumn();
                ImGui::Text("Player 2"); ImGui::NextColumn();
                ImGui::Text("Ping"); ImGui::NextColumn();
        ...
        if (active) {
            ...
            if (!joining) {
                if (InputManager->P1.Yaxis == 1) {
                    ip_id = (ip_id + 1) % hosts[page_id].size();
                    ...
                    ip_id -= 1;
                    if (ip_id < 0) ip_id = hosts[page_id].size() - 1;
                    InputManager->P1.Yaxis = -10;
